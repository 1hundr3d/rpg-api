...
if DATABASE_URL is None:
    raise ValueError(
        "DATABASE_URL is not set! Please add it to Render environment variables."
    )

if SECRET_KEY is None:
    raise ValueError(
        "SECRET_KEY is not set! Please add it to Render environment variables."
    )
...
def init_db():
    ...
    if not inspector.has_table("heroes"):
